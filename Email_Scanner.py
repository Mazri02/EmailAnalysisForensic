...
##############################################################################
def write_data_to_excel(data, excel_filename):
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Email Analysis"

    # Write Introduction Banner
    ws.merge_cells("A1:B1")
    ws["A1"] = "Email Analysis"
    ...
    if data["Analysis"].get("Headers"):
        ...
        if data["Analysis"]["Headers"].get("Investigation"):
            ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=2)
            ws[f"A{row}"] = "Header Investigation"
            ws["A" + str(row)].font = Font(bold=True)
            row += 1
    ...
    if data["Analysis"].get("Digests"):
        ...
        if data["Analysis"]["Digests"].get("Investigation"):
            ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=2)
            ws[f"A{row}"] = "Digest Investigation"
            ws["A" + str(row)].font = Font(bold=True)
            row += 1
    ...
    if data["Analysis"].get("Attachments"):
        ...
        if data["Analysis"]["Attachments"].get("Investigation"):
            ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=2)
            ws[f"A{row}"] = "Attachment Investigation"
            ws["A" + str(row)].font = Font(bold=True)
            row += 1
...
def upload_to_virustotal(filename):
    with open(filename, "rb") as f:
...
def get_virustotal_report(file_id):
    client = vt.Client(VIRUSTOTAL_API_KEY)
    while True:
        analysis = client.get_object(f"/analyses/{file_id}")
...
if __name__ == '__main__':
    ...
    if args.filename:
        ...
        file_id = upload_to_virustotal(filename)
        virustotal_report = get_virustotal_report(file_id)
        print(f"VirusTotal Report: {virustotal_report}")
    ...
    if any(arg_list):
        ...
        if args.output:
            ...
            write_data_to_excel(app_data, 'Email Analysis.xlsx')
